...
export const Pong = {
  ...
  draw: function (ctx) {
    ...
    if (this.playing) { this.ball.draw(ctx) } else { this.menu.draw(ctx) }
  },
  ...
  Ball: {
    ...
    update: function (dt, leftPaddle, rightPaddle) {
      const pos = Pong.Helper.accelerate(this.x, this.y, this.dx, this.dy, this.accel, dt)
      ...
      if (pt) {
        ...
        if (paddle.up) { pos.dy = pos.dy * (pos.dy < 0 ? 0.5 : 1.5) } else if (paddle.down) { pos.dy = pos.dy * (pos.dy > 0 ? 0.5 : 1.5) }
      }
    ...
    draw: function (ctx) {
      ...
      ctx.fillRect(this.x - this.radius, this.y - this.radius, w, w)
      if (this.pong.cfg.footprints) {
        var max = this.footprints.length
        ctx.strokeStyle = Pong.Colors.footprint
        for (var n = 0; n < max; n++) { ctx.strokeRect(this.footprints[n].x - this.radius, this.footprints[n].y - this.radius, w, w) }
      }
    }
